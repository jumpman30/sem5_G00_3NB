...
export default class TaskRepo implements ITaskRepo {
  ...
  public async save(taskData: Task): Promise<{ warning?: string; task: Task }> {
    ...
    switch (taskData.type) {
      case TaskType.Surveillance: {
        ...
        if (existingTask) {
          return {
            warning: 'Surveillance task already exists',
            task: SurveillanceTaskMap.toDomain(existingTask),
          };
        } else {
      ...
      case TaskType.PickupAndDelivery: {
        ...
        if (existingTask) {
          return {
            warning: 'Pickup and Delivery task already exists',
            task: PickupDeliveryTaskMap.toDomain(existingTask),
          };
        } else {
          const rawTask = PickupDeliveryTaskMap.toPersistence(taskData);
          const taskCreated = await this.pickupDeliveryTaskSchema.create(
            rawTask,
          );
  ...
  async getAllTasks() {
    ...
    return [
      ...((SurveillanceTaskMap.toDomain(
        surveillanceTasks,
      ) as unknown) as Task[]),
      ...((PickupDeliveryTaskMap.toDomain(
        pickupDeliveryTasks,
      ) as unknown) as Task[]),
    ];
  }
  ...
  public async update(task: Task): Promise<Task> {
    try {
      ...
      switch (taskType) {
        case TaskType.Surveillance: {
          const taskRecord = await this.surveillanceTaskSchema.findByIdAndUpdate(
            { taskId },
            SurveillanceTaskMap.toPersistence(task),
            { new: true },
          );
          if (taskRecord != null) {
            return SurveillanceTaskMap.toDomain(taskRecord);
        ...
        case TaskType.PickupAndDelivery: {
          const taskRecord = await this.pickupDeliveryTaskSchema.findByIdAndUpdate(
            { taskId },
            SurveillanceTaskMap.toPersistence(task),
            { new: true },
          );
  ...
  async delete(task: Task): Promise<Task | null> {
    try {
      ...
      switch (taskType) {
        case TaskType.Surveillance: {
          const taskRecord = await this.surveillanceTaskSchema.findByIdAndDelete(
            taskId,
          );
          if (taskRecord) {
            return SurveillanceTaskMap.toDomain(taskRecord);
        ...
        case TaskType.PickupAndDelivery: {
          const taskRecord = await this.pickupDeliveryTaskSchema.findByIdAndDelete(
            taskId,
          );
          if (taskRecord) {
            return PickupDeliveryTaskMap.toDomain(taskRecord);
  ...
  public async exists(task: TaskId | string): Promise<boolean> {
    ...
    const idX = task instanceof TaskId ? (<TaskId>task).toValue() : task;
  ...
  public async findByTaskId(taskId: TaskId | string): Promise<Task> {
    const taskRecord = await this.surveillanceTaskSchema.findOne({
      taskId: taskId,
    });
    if (taskRecord != null) {
    ...
    try {
      switch (status) {
        ...
        case TaskStatus.InProgress:
          // Consulta para buscar tarefas pelo status no esquema de vigilância
          const surveillanceTasks = await this.surveillanceTaskSchema.find({
            taskStatus: status,
          });

          // Consulta para buscar tarefas pelo status no esquema de coleta/entrega
          const pickupDeliveryTasks = await this.pickupDeliveryTaskSchema.find({
            taskStatus: status,
          });

          // Mapeia as tarefas para domínio e junta as duas listas
          const tasks = [
            ...((SurveillanceTaskMap.toDomain(
              surveillanceTasks,
            ) as unknown) as Task[]),
          ...
          return tasks;
        default:
          // Retorna um array vazio se o status não for reconhecido
          return [];
      }
    } catch (e) {
      // TODO: Gerenciar erro de maneira apropriada
      throw e;
    }
  }
}
